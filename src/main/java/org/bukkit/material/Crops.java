...
public class Crops extends MaterialData {
    ...
    public CropState getState() {
        ...
        CropState state = switch (type) {
            case CROPS, CARROT, POTATO ->
                // Mask the data just in case top bit set
                    CropState.getByData((byte) (getData() & 0x7));
            case BEETROOT_BLOCK, NETHER_WARTS ->
                // Mask the data just in case top bits are set
                // Will return SEEDED, SMALL, TALL, RIPE for the three growth data values
                    CropState.getByData((byte) (((getData() & 0x3) * 7 + 2) / 3));
